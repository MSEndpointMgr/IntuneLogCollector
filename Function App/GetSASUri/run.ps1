...
if ($HeaderValidation -eq $true) {
    ...
    if ($AzureADDeviceRecord -ne $null) {
        ...
        if (Test-AzureADDeviceAlternativeSecurityIds -AlternativeSecurityIdKey $AzureADDeviceRecord.alternativeSecurityIds.key -Type "Thumbprint" -Value $Thumbprint) {
            ...
            if (Test-AzureADDeviceAlternativeSecurityIds -AlternativeSecurityIdKey $AzureADDeviceRecord.alternativeSecurityIds.key -Type "Hash" -Value $PublicKey) {
                ...
                if ($EncryptionVerification -eq $true) {
                    ...
                    if ($AzureADDeviceRecord.accountEnabled -eq $true) {
                        ...
                        $BodyTable = New-ErrorResponse -Code "Forbidden" -Message "Trusted Azure AD device record validation for inbound request failed, record with deviceId '$($DeviceID)' is disabled"
                    }
                }
                else {
                    ...
                    $StatusCode = [HttpStatusCode]::Forbidden
                    $BodyTable = New-ErrorResponse -Code "Forbidden" -Message "Trusted Azure AD device record validation for inbound request failed, could not validate signed content from client"
                }
            }
            else {
                ...
                $StatusCode = [HttpStatusCode]::Forbidden
                $BodyTable = New-ErrorResponse -Code "Forbidden" -Message "Trusted Azure AD device record validation for inbound request failed, could not validate certificate SHA256 hash value"
            }
        }
        else {
            ...
            $StatusCode = [HttpStatusCode]::Forbidden
            $BodyTable = New-ErrorResponse -Code "Forbidden" -Message "Trusted Azure AD device record validation for inbound request failed, could not validate certificate thumbprint"
        }
    }
    else {
        ...
        $StatusCode = [HttpStatusCode]::Forbidden
        $BodyTable = New-ErrorResponse -Code "Forbidden" -Message "Trusted Azure AD device record validation for inbound request failed, could not find device with deviceId: $($DeviceID)"
    }
}
